...
contract TournamentManager is ReentrancyGuard {
    ...
    function leaveTournament(uint256 tournamentId) external nonReentrant {
        Tournament storage t = tournaments[tournamentId];
        
        if (t.state != TournamentState.Open) revert TournamentNotOpen();
        if (!isPlayerInTournament[tournamentId][msg.sender]) revert("Not joined");

        // Refund logic
        if (t.entryFee > 0) {
             _pushFunds(t.entryToken, msg.sender, t.entryFee);
             t.totalPrizePool -= t.entryFee;
        }
        ...
        for (uint i = 0; i < t.players.length; i++) {
            if (t.players[i] == msg.sender) {
                t.players[i] = t.players[t.players.length - 1];
                t.players.pop();
                found = true;
                break;
            }
        ...
        if (found) {
            t.playerCount--;
            isPlayerInTournament[tournamentId][msg.sender] = false;
            emit PlayerLeft(tournamentId, msg.sender);

            // If empty, cancel tournament
            if (t.playerCount == 0) {
    ...
    // Allow creator to cancel if not full
    function cancelTournament(uint256 tournamentId) external nonReentrant {
        ...
        for (uint i = 0; i < t.players.length; i++) {
            _pushFunds(t.entryToken, t.players[i], t.entryFee);
             isPlayerInTournament[tournamentId][t.players[i]] = false;
        }
